...
std::vector<validation_rule*> validation_parser::parse_double_field(
  ...
  (void)type;
  std::vector<validation_rule*> rules;
  add_double_rule(rules, "vt.const", annotations);
  ...
  add_double_rule(rules, "vt.in", annotations);
  add_double_rule(rules, "vt.not_in", annotations);
  return rules;
}
...
std::vector<validation_rule*> validation_parser::parse_integer_field(
  ...
  (void)type;
  std::vector<validation_rule*> rules;
  add_integer_rule(rules, "vt.const", annotations);
  ...
  add_integer_rule(rules, "vt.in", annotations);
  add_integer_rule(rules, "vt.not_in", annotations);
  return rules;
}
...
std::vector<validation_rule*> validation_parser::parse_string_field(
  ...
  add_string_rule(rules, "vt.in", annotations);
  add_string_rule(rules, "vt.not_in", annotations);
  return rules;
}
...
std::vector<validation_rule*> validation_parser::parse_map_field(
  ...
  key_rules = parse_field(((t_list*)type)->get_elem_type(), key_annotations);
  ...
  value_rules = parse_field(((t_list*)type)->get_elem_type(), value_annotations);
  ...
  if (it != annotations.end() && !it->second.empty()) {
    ...
    for (auto& annotation_value : it->second) {
      ...
      if (is_reference_field(annotation_value)) {
        ...
        bool constant;
        std::istringstream(it->second.back()) >> constant;
