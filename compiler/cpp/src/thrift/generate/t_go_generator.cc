...
void t_go_generator::generate_go_struct(t_struct* tstruct, bool is_exception) {
  ...
  // genrate IsValid function
  if (generate_validator) {
    std::string tstruct_name(publicize(tstruct->get_name(), false));
    f_types_ << "func (p *" << tstruct_name << ") IsValid() error {" << endl;
    t_go_validator_generator(this).generate_struct_validator(f_types_, tstruct);
    f_types_ << "return nil" << endl;
    f_types_ << "}" << endl;
  }
}
...
THRIFT_REGISTER_GENERATOR(go, "Go",
                          ...
                          "    generate_validator\n"
                          "                     Generate IsValid method for struct/union/exception\n")
